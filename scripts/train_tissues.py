...
def _pt_to_npy(pt_path):
	...
	# Use a lock file to prevent concurrent conversion
...
def convert_all_tracks(training_tracks, args):
	if _LOCAL_RANK == 0:
		...
		for t in training_tracks:
			...
			inp_pt = f"{td}/{sp}/{ti}/{bn}_rna_{args.region_len}_log_rnaseq_final.pt"
			out_pt = (f"{td}/{sp}/{ti}/{bn}_{tag}_"
					  f"{args.region_len}_{args.nBins}_log_riboseq_final.pt")
			inp_npy = inp_pt[:-3] + ".npy"
			out_npy = out_pt[:-3] + ".npy"
			if not os.path.exists(inp_npy) and os.path.exists(inp_pt):
				_pt_to_npy(inp_pt)
			if not os.path.exists(out_npy) and os.path.exists(out_pt):
				_pt_to_npy(out_pt)
...
def split_chromosomes(chroms, trial=None, fixed_val=None):
	"""
	Split chromosomes into (train, val, test).
	If trial="X,Y,Z": train on chr X only, val on chr Y, test on chr Z.
	"""
...
def _randomize_seq(seq, seq_len):
	"""Replace non-N bases with random A/C/T/G, keeping Ns unchanged."""
	s = seq[:seq_len]
	arr = np.array(list(s))
	non_n = (arr != 'N') & (arr != 'n')
	n_replace = non_n.sum()
...
class TissueRiboDataModule(LightningDataModule):
	...
	def __init__(self, args, training_tracks, val_chroms, tissue_vocab,
		...
		self.test_tracks     = test_tracks or []
	...
	def setup(self, stage=None):
		...
		if _RANK == 0:
			_dbg(f"Total: {total_tr} train, {total_vl} val, {total_te} test (hold-out)")
		print(f"\n  Total dataset: {total_tr} train, {total_vl} val, "
			  f"{total_te} test (hold-out chroms)")
		print(f"  Tissues: {len(self.tissue_vocab)}")

		# ── Load test-tagged tracks ONLY if --test is used ─────────────────
		if self.args.test and self.test_tracks:
			...
			for track in self.test_tracks:
				...
				self.test_new_track_names.append((sp, ti))
				...
				if not os.path.exists(inp_npy) or not os.path.exists(out_npy):
					if _RANK == 0:
						_dbg(f"SKIP test track {sp}/{ti}: missing npy"); continue

				df_all = load_coords(sp)
				if df_all is None or df_all.empty:
					if _RANK == 0:
						_dbg(f"SKIP test track {sp}/{ti}: no coordinates"); continue
				...
				if inp_mm.shape[0] != len(df_all):
					if _RANK == 0:
						_dbg(f"SKIP test track {sp}/{ti}: shape mismatch"); continue
				...
				if len(all_idx):
					ds = TissueTranscriptDataset(
						sequences, all_idx, inp_mm, out_mm,
						tissue_id, self.args.region_len, self.args.nBins,
						self.args.nosequence, track_id=test_new_track_id)
					test_new_parts.append(ds)
					if _RANK == 0:
						_dbg(f"LOADED test track {sp}/{ti} (all chroms): n={len(ds)}")
			...
			print(f"  Test-tagged tracks: not loaded (use --test to load)\n")
	...
	def test_dataloader(self):
		"""Return multiple test dataloaders."""
		dataloaders = []
		
		if self._test_ds is not None and len(self._test_ds) > 0:
			dataloaders.append(self._make_loader(self._test_ds, shuffle=False, drop_last=True))
		
		if self._test_new_ds is not None and len(self._test_new_ds) > 0:
			dataloaders.append(self._make_loader(self._test_new_ds, shuffle=False, drop_last=True))
...
class TissueRiboModel(LightningModule):
	...
	def test_step(self, batch, batch_idx, dataloader_idx=0):
		"""Test step with dataloader index to distinguish between test sets."""
		...
		# Set test phase based on dataloader index
		self._test_phase = "holdout" if dataloader_idx == 0 else "new_samples"
		
		# Use appropriate track names
		if dataloader_idx == 0:
			self._track_names = self.trainer.datamodule.track_names
		else:
			self._track_names = self.trainer.datamodule.test_new_track_names
	...
	def on_test_epoch_end(self):
		...
		if self.global_rank == 0:
			print(f"\n  {'='*60}")
			if is_holdout:
				print(f"  Test results — hold-out chromosomes (all training tracks)")
			else:
				print(f"  Test results — new samples (test-tagged tracks, all chroms)")
			print(f"  {'='*60}")
		...
		for tid in sorted(self._test_per_track.keys()):
			...
			self.log(f"test_{phase}/loss_{label}", avg_loss, sync_dist=False)
			self.log(f"test_{phase}/pcc_{label}",  pcc,      sync_dist=False)
		...
		if self.global_rank == 0 and results:
			...
			csv_path = os.path.join(
				self.args.save_dir, f"test_results_{phase}.csv")
...
def parse_args():
	...
	p.add_argument("--model-type",     default="PosTransModelTCNFiLMRef",
				   choices=["PosTransModelTCNFiLM", "PosTransModelTCNFiLMRef", "TransModelFiLM"]),
	...
	p.add_argument("--zero_w",          type=float, default=0.1,
				   help="Loss weight for zero-target bins (default: 0.1)")
	p.add_argument("--pcc_loss_w",      type=float, default=0.2,
				   help="Weight of per-sample PCC loss term (default: 0.2)")
...
def main():
	...
	test_tracks     = [t for t in all_tracks if t["dataset"] == "test"]
	...
	save_dir = (
		...
		f"_nt{len(tissue_vocab)}_{args.model_type}_seed{args.seed}{suffix}"
	)
	...
	if _RANK == 0:
		print(f"\n{'='*60}")
		print(f"  RiboTransPred - Tissue FiLM  |  {args.model_type}")
	...
	# In test mode, auto-find checkpoint if not explicitly given
	if args.test and ckpt is None:
		...
		if os.path.exists(last_ckpt):
			...
			# Try best checkpoint by modification time
			import glob
			ckpts = sorted(glob.glob(os.path.join(save_dir, "epoch=*.ckpt")),
